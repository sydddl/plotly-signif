...
class plotly_signif():
    ...
    def add_annotations(self,annotation_list=list(),
                        text="***",size=16,color = 'rgba(0,0,0,1)',inne_color="rgba(20,20,20,0.7)",line_width=2,  # 传到annotation的
                        y_max_index = 1.15,part_down_index = 0.08,sign_d_index = 4,line_d_index = 3.2,
                        ):
        '''
        ...
        part_down = bar_max * part_down_index
        d = part_down / sign_d_index
        line_d = part_down / line_d_index
    ...
    def annotation(self,anno_at,inne=False,**kwargs):
        ...
        if inne is True:
            ...
            self.fig.add_annotation(text=kwargs["text"],name="p-value",xref="x", yref="y",
                x=group_at + (diff1+diff2)/2 , y=y_max, showarrow=False,font=dict(size=kwargs["size"], color=kwargs["inne_color"]),)
