...
class Plot_Box_with_annotation():
    def __init__(self,data=pd.DataFrame,error=pd.DataFrame,text={},json={}) -> None:
        ...
        self.color = ["#5044f3",'#8ca0f7',"rgba(104, 135, 255,0.4)",'#6DECB9', "rgba(154, 225, 155,0.4)"]  #默认颜色

    def plot(self,pic_px=[500,550],save_path=None,scale=400/96):
        ...
        self.fig.update_yaxes(title_text=self.text["y"],range=[0, 1],showline=True, linewidth=3, linecolor='black', tickfont=dict(size=16),
    ...
    def add_annotations(self,annotation_list=list(),
                        text="***",size=17,color = 'rgba(0,0,0,1)',inne_color="rgba(0,0,0,0.5)",line_width=2,d = 0.05,line_d = 0.02,  # 传到annotation的
                        part_down = 0.08,y_max = 1.02):
        '''
        ...
        part_down : 上下间距, y_max : 最上层y轴位置
        '''
        for i in annotation_list:
            y_layer = i[-1]
            y = y_max - part_down * y_layer
    ...
    def annotation(self,anno_at,inne=False,**kwargs):
        ...
        if inne is True:
            ...
            self.fig.add_annotation(text=kwargs["text"],name="p-value",xref="x", yref="paper",
                x=group_at + (diff1+diff2)/2 , y=y_max, showarrow=False,font=dict(size=kwargs["size"] - 3, color=kwargs["inne_color"]),)
            self.fig.add_shape(type="line",x0=group_at + diff1,y0=y_max-d, x1=group_at + diff2, y1=y_max-d,
                               line=dict(color=kwargs["inne_color"],width=kwargs["line_width"]))
            self.fig.add_shape(type="line",x0=group_at + diff1,y0=y_max-d-line_d, x1=group_at + diff1, y1=y_max-d,
                               line=dict(color=kwargs["inne_color"],width=kwargs["line_width"]))
            self.fig.add_shape(type="line",x0=group_at + diff2,y0=y_max-d-line_d, x1=group_at + diff2, y1=y_max-d,
                               line=dict(color=kwargs["inne_color"],width=kwargs["line_width"]))
        else:
            x_local = np.array(anno_at).mean()
            self.fig.add_annotation(text=kwargs["text"],name="p-value",xref="x", yref="paper",
                        x=x_local, y=y_max, showarrow=False,font=dict(size=kwargs["size"], color="black"),)
            self.fig.add_shape(type="line",x0=self.Group_name[anno_at[0]],y0=y_max-d, x1=self.Group_name[anno_at[1]], y1=y_max-d,
                               line=dict(color=kwargs["color"],width=kwargs["line_width"]))
            self.fig.add_shape(type="line",x0=self.Group_name[anno_at[0]],y0=y_max-d-line_d, x1=self.Group_name[anno_at[0]], y1=y_max-d,
                               line=dict(color=kwargs["color"],width=kwargs["line_width"]))
            self.fig.add_shape(type="line",x0=self.Group_name[anno_at[1]],y0=y_max-d-line_d, x1=self.Group_name[anno_at[1]], y1=y_max-d,
                               line=dict(color=kwargs["color"],width=kwargs["line_width"]))
